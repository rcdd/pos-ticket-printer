...
export default function SessionPage({ session, setSession, onCloseSession }) {
    ...
    const handleCloseSession = async (notes) => {
        ...
        const {
            ...
            finalCashValueCents
        } = computeAggregates(invoices, session?.initialAmount ?? 0, cashMovements);
    ...
    const {
        ...
        cashOut,
        netAdjustments,
    } = useMemo(() => computeAggregates(invoices, session?.initialAmount ?? 0, cashMovements), [invoices, session?.initialAmount, cashMovements]);
...
function computeAggregates(invoices, initialAmountCents, cashMovements) {
    let totalAmount = 0;
        ...
        for (const rec of inv?.records ?? []) {
            ...
            if (hasDiscount) {
                ...
                const prev = discountedMap.get(key) ?? {
                    id: p.id, name: p.name, zone: p.zone, quantity: 0, total: 0, discount: discPct,
                };
                prev.quantity += qty;
                prev.total += discountedPrice * qty;
                discountedMap.set(key, prev);
            } else {
                const prev = productsMap.get(p.id) ?? { id: p.id, name: p.name, zone: p.zone, quantity: 0, total: 0 };
                prev.quantity += qty;
