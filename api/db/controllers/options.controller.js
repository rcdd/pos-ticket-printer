...
const optionPrintMethod = 'printMethod';
const optionDirectPrintConfig = 'directPrintConfig';
...
export const getPrintMethod = async (req, res) => {
    try {
        const row = await Option.findOne({where: {name: optionPrintMethod}});
        const method = row?.value ?? 'shared'; // Default to 'shared'
        res.send({printMethod: method});
    } catch (error) {
        res.status(500).send({
            message: "Error retrieving print method.",
            error: error?.message || error,
        });
    }
};

export const setPrintMethod = async (req, res) => {
    try {
        const {printMethod} = req.body;

        if (!printMethod || !['shared', 'direct'].includes(printMethod)) {
            return res.status(400).send({
                message: "Invalid print method. Must be 'shared' or 'direct'."
            });
        }

        const [option] = await Option.findOrCreate({
            where: {name: optionPrintMethod},
            defaults: {value: printMethod},
        });

        if (option.value !== printMethod) {
            await option.update({value: printMethod});
        }

        res.send({printMethod});
    } catch (error) {
        res.status(500).send({
            message: "Error saving print method.",
            error: error?.message || error,
        });
    }
};

export const getPrintMethodVariable = async () => {
    const row = await Option.findOne({where: {name: optionPrintMethod}});
    return row?.value ?? 'shared';
};

// Direct Print Configuration management
export const getDirectPrintConfig = async (req, res) => {
    try {
        const row = await Option.findOne({where: {name: optionDirectPrintConfig}});

        if (!row || !row.value) {
            // Default configuration for network printer
            return res.send({
                config: {
                    type: 'network',
                    ip: '',
        ...
        res.status(500).send({
            message: "Error retrieving direct print configuration.",
            error: error?.message || error,
        });
    }
};

export const setDirectPrintConfig = async (req, res) => {
    try {
        const {config} = req.body;

        if (!config || !config.type) {
            return res.status(400).send({
                message: "Invalid configuration. 'type' is required."
            });
        }

        // Validate type is one of the allowed values
        if (!['network', 'usb', 'serial'].includes(config.type)) {
            return res.status(400).send({
                message: "Invalid type. Must be 'network', 'usb', or 'serial'."
            });
        }

        // Allow saving incomplete configs - validation happens when printing
        // This allows users to change connection type before filling in details

        const configString = JSON.stringify(config);

        const [option] = await Option.findOrCreate({
            where: {name: optionDirectPrintConfig},
            defaults: {value: configString},
        });

        if (option.value !== configString) {
            await option.update({value: configString});
        }

        res.send({config});
    } catch (error) {
        res.status(500).send({
            message: "Error saving direct print configuration.",
            error: error?.message || error,
        });
    }
};

export const getDirectPrintConfigVariable = async () => {
    try {
        const row = await Option.findOne({where: {name: optionDirectPrintConfig}});
        ...
        console.error('Error parsing direct print config:', error);
        return null;
    }
};
